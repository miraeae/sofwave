...
GSAP 라이브러리를 활용하여 인터렉티브한 요소를 중점적으로 구현한 클론 코딩 프로젝트입니다.
...
### 1. 인트로 하루에 한번만 활성화
사용자의 편의성을 위해 하루에 한 번만 인트로가 활성화되도록 쿠키를 활용하여 제어했다.

![Image](https://github.com/user-attachments/assets/a242dd25-130f-46e7-8a9e-d2b83d54436e)

먼저, ```setCookie(name, value, days)``` 함수를 사용하여 쿠키를 설정했다. 이 함수는 쿠키의 이름, 값, 그리고 만료 기간을 지정하며, 설정된 기간(일 단위) 후 만료되도록 한다.
```
...
* (1000 * 60 * 60 * 24) 는 밀리초를 날짜로 변환하기 위해 사용되며, 1일은 24시간 * 60분 * 60초 * 1000밀리초 로 표현된다.

<br/>

쿠키 값을 가져오는 ```getCookie(name)``` 함수로 저장된 쿠키 목록에서 특정 쿠키 값을 찾아 반환한다.
```
...
위 두 함수를 활용하여, 첫 방문 시 visited 쿠키를 저장하게 했다.
```
...
로컬에선 확인이 불가하기 때문에 깃허브 페이지에 push 하여 쿠키가 잘 저장되는 것을 확인했다. 이렇게 저장된 쿠키 존재 여부에 따라 인트로 유무를 판단할 수 있는 로직을 작성했다.
...
추가로 Intro 애니메이션엔 ```onComplete```를 사용하여 끝났을때 Hero 애니메이션이 실행되게 했다.
```
    introTl
    ···
...
솔루션 섹션에 효과가 PC와 Mobile이 달랐다. 이를 구현하기위해 GSAP 반응형인 matchMedia를 사용했다.
- PC: 타이틀 고정, 직사각형이 커짐
...
***
### 3. sticky를 이용한 pin효과
해당 사이트는 섹션마다 배경이 고정되는 효과가 많다. GSAP ScrollTrigger의 Pin을 사용할 수 있지만 간단하게 구현하고 싶은 곳은 <code>position:sticky</code>을 사용했다.

- sticky: 최초 relative속성과 같이 동작하다가, 스크롤시 지정 지점에서 요소를 고정시킴

sticky는 주의할 점이 부모 요소 중에 overflow: hidden, auto, scroll 속성이 적용되어 있으면 안 된다. 또한, 부모의 height 값만큼 고정이 되기 때문에 부모 태그가 높이 값이 없다면 고정되지 않는다. 예를 들어 자식 태그에 absolute 속성을 사용하여 부모 태그가 height 값이 잡히지 않는 경우엔 따로 높이 값을 지정해 주어야 한다.
```
.info {
    &__bg{
        height:100vh;
...
반복되는 효과엔 반복문을 사용하여 코드 길이를 줄였다. 마지막 아이템만 다음 아이템으로 가는 과정이 없기 때문에 if문을 사용하여 따로 작성해 줬다.
```
    for (let i = 1; i < 4; i++) {
        if(i < 3){
            effectTl
